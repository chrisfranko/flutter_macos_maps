    ...
    body: Column(children: [
      Expanded(
        child: FlutterMacosMapsView(
          initialCamera: CameraPosition(target: const LatLng(37.3349, -122.0090), latDelta: 0.05, lonDelta: 0.05),
          onCreated: (c) {
            ctrl = c;
            // listen to events
            c.onTap.listen((pos) => debugPrint('tap: ${pos.lat}, ${pos.lon}'));
            c.onAnnotationTap.listen((id) => debugPrint('annotation tapped: $id'));
            c.onRegionChanged.listen((e) => debugPrint('region changed: ${e.camera.target.lat}'));
          },
        ),
      ),
